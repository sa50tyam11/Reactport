...
class AnimationController {
  ...
  render() {
    ...
    // Clear canvas entirely to let the bright CSS background blend
  ...
  drawTrail(t1) {
    for (let i = 0; i < this.trailLength; i++) {
      ...
      // Much brighter, fully opaque purple for the core tail
      this.ctx.fillStyle = 'rgba(192, 132, 252, 1)'; 
      this.ctx.lineWidth = sw;
...
class Star {
  ...
  render(p, controller) {
    ...
    if (q > 0) {
      ...
      // Increased base particle size so it glows brighter
      const dotSize = 10.0 * this.strokeWeightFactor * sizeMultiplier;
      
      // Brighter colors for the stars
      if (this.strokeWeightFactor > 0.6) {
        controller.ctx.fillStyle = '#d8b4fe'; // Brighter purple
      } else {
        controller.ctx.fillStyle = 'rgba(255, 255, 255, 0.95)'; // Brighter white
      }
...
export function SpiralAnimation() {
  ...
  return (
    // Changed opacity-50 to opacity-100 to make the glow extremely prominent
    <div className="absolute inset-0 w-full h-full overflow-hidden opacity-100 mix-blend-screen pointer-events-none z-0">
      <canvas
        ref={canvasRef}
